...
if [[ ! -f "./hytale-downloader/hytale-downloader-linux" ]]; then
    echo "Error: Hytale Downloader not found! Проверьте скрипт установки в панели."
    exit 1
fi
...
if [[ -z "$HYTALE_SERVER_SESSION_TOKEN" ]]; then
    if [[ ! -d "Server" || "${FORCE_UPDATE}" == "1" ]]; then
        ...
        echo "Hytale server files found. Skipping download."
    fi
fi
if [[ "${INSTALL_SOURCEQUERY_PLUGIN}" == "1" ]]; then
    if [[ ! -f "mods/hytale-sourcequery.jar" || "${FORCE_UPDATE}" == "1" ]]; then
        mkdir -p mods
        echo "Updating/Installing SourceQuery plugin..."
